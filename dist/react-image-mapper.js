	...
	_createClass(ImageMapper, [{
		...
		value: function click(area, index, event) {
			if (this.props.onClick) {
				...
				var shape = event.target.getAttribute("shape");
				var previousSelection = this.state.areasClicked;
				var currentSelection = [];

				this.props.onClick(area, index, event);

				if (previousSelection.includes(index)) {
					currentSelection = previousSelection.filter(function (areas) {
						return areas !== index;
					});
		...
		key: "removeClickFill",
		value: function removeClickFill(coords) {
			var _coords2 = _slicedToArray(coords, 4);

			var left = _coords2[0];
			var top = _coords2[1];
			var right = _coords2[2];
			var bot = _coords2[3];
...
ImageMapper.propTypes = {
	...
	areasClicked: _propTypes2["default"].array,
